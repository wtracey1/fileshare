...
cocos2d::Vec2 AgentSprite::computeWallAvoidance(){
  ...
  if ( position.x > viewSize.width - width/2 - wallSight ) { //within sight of right wall
    distanceToWall = viewSize.width - width/2 - position.x;
    forceStrength = 2.0/(distanceToWall+.1);
    force.x = force.x - forceStrength;
    //force.y = force.y + (force.y>0? forceStrength : -forceStrength
  }
  ...
  if ( position.y > viewSize.height - height/2 - wallSight ) {  //within sight of top wall
    distanceToWall = viewSize.height - position.y - height/2;
    forceStrength = 2.0/(distanceToWall+.1);
  ...
  std::cout << "wallForce: (" << force.x << ", " << force.y << ")" << std::endl;
  return force;
}
/*
