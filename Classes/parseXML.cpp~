...
void ParseXML::parseXML() {
  ...
  if (!in) { 
    //Hack to work when running through valgrind on my own computer for debugging
    in.open("/home/wtracey/class/project4/Resources/xmlSpec/game.xml",std::ios::in);
    if(!in) throw string("Cannot open xml file: ")+filename;
  }
